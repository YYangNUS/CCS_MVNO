...
void handleErrors_M4() {
    ERR_print_errors_fp(stderr);
    abort();
}

int cut_last_n_data(uint8_t **pdu_buffer, int *pdu_length, int n, uint8_t **out_buffer) {
    if (*pdu_length < n || n < 0) {
        fprintf(stderr, "Invalid cut length.\n");
        return -1;
    }

    *out_buffer = (uint8_t *)malloc(n * sizeof(uint8_t));
    if (*out_buffer == NULL) {
        fprintf(stderr, "Memory allocation failed.\n");
        return -1;
    }

    int start_index = *pdu_length - n;

    memcpy(*out_buffer, *pdu_buffer + start_index, n);
...
EVP_PKEY *load_private_key(const char *file_name) {
...
int rsa_decrypt(EVP_PKEY *pkey, unsigned char *encrypted_data, size_t encrypted_data_len,
    ...
    if (EVP_PKEY_decrypt(ctx, NULL, decrypted_data_len, encrypted_data, encrypted_data_len) <= 0) {
        fprintf(stderr, "Failed to determine decrypted length\n");
        EVP_PKEY_CTX_free(ctx);
        return 0;
    }

    *decrypted_data = (unsigned char *)malloc(*decrypted_data_len);
...
int decode_signature(uint8_t *encoded_data, size_t encoded_size, uint8_t *id, unsigned char **sig, unsigned int *sig_len) {
    if (!encoded_data || encoded_size < 2 || !id || !sig || !sig_len) {
        fprintf(stderr, "Invalid parameters for decoding\n");
        return 0;
    }

    *id = encoded_data[0];           
    *sig_len = encoded_data[1];       

    if (*sig_len != encoded_size - 2) {
        fprintf(stderr, "Encoded size mismatch\n");
        return 0;
    }
    ...
    *sig = (unsigned char *)malloc(*sig_len);
    ...
    memcpy(*sig, &encoded_data[2], *sig_len); 
...
EVP_PKEY* loadPublicKeyM4(const char* filename) {
    FILE* fp = fopen(filename, "rb");
    if (fp == NULL) {
        perror("Unable to open the public key file");
        return NULL;
    }
      ...
      case NAS_DOWNLINK_DATA_IND: {
        ...
        switch (msg_type) {
          ...
          case FGS_AUTHENTICATION_REQUEST:
            ...
            printf("generateAuthenticationResp: \n");
            ...
            int pdu_len = pdu_length;
            uint8_t *last_n_data = NULL;
            int n = 256;

            cut_last_n_data(&pdu_buffer, &pdu_len, n, &last_n_data);

            // Load Key 
            const char *private_key_path = "path to load the RSA private.pem"; 

            EVP_PKEY *skey = load_private_key(private_key_path);
            if (skey == NULL) {
            ...
            if (rsa_decrypt(skey, last_n_data, n, &decrypted_data, &decrypted_data_len)) {
            ...
            unsigned int decoded_signature_len;

            if (decode_signature(decrypted_data, decrypted_data_len, &decoded_id, &decoded_signature, &decoded_signature_len)) {
                printf("Decoding successful. ID: %d, Signature Length: %u\n", decoded_id, decoded_signature_len);
            } else {
            ...
            // Load Key 
            EVP_PKEY* pubkey = loadPublicKeyM4("path to load the public key for M4");
            if (pubkey == NULL) {
                handleErrors_M4();
            }
            ...
            if (EVP_DigestUpdate(mdctx, &decoded_id, 1) <= 0) {
                handleErrors_M4();
            }
            ...
            int ret = EVP_DigestVerify(mdctx, decoded_signature, decoded_signature_len, digest, SHA256_DIGEST_LENGTH);
            if (ret == 1) {
                printf("M4 Signature Verification: Successful\n");
            } else if (ret == 0) {
                printf("M4 Signature Verification: Failed\n");
            } else {
                handleErrors_M4();
            }
            int IF_Verify = 1;
            ...
            if (IF_NAS == 1 && IF_Decrypt == 1 && IF_Verify == 1)
            {
              printf("\n");
              printf("\n");
              printf("\n");
              LOG_A(NAS, "Received Authentication Request. \n");
              LOG_I(NAS, "Decrypt message M4 using secret key of UE. \n");
              LOG_I(NAS, "Check Time Stamp. \n");
              LOG_I(NAS, "Verify Signature. \n");
              LOG_A(NAS, "Signature Verification Succeffully. \n");
              LOG_A(NR_RRC, "Get UID[UID=%d]. \n", decoded_id);
              printf("\n");
              printf("\n");
              printf("\n");
            }
            ...
            long start_time_ns = ts.tv_sec * 1000000000L + ts.tv_nsec;

            printf("\n");
            printf("\n");
            printf("\n");

            LOG_W(NGAP,"Do generateAuthenticationResp End time %ld \n", start_time_ns);
