...
uint8_t* read_uint8_from_bin_file(const char* filename, size_t* size) {
    ...
    *size = ftell(file);
    ...
    size_t read_size = fread(buffer, sizeof(uint8_t), *size, file);
    if (read_size != *size) {
...
//------------------------------------------------------------------------------
{
  ...
  {
    asn1cSequenceAdd(out->protocolIEs.list, NGAP_InitialUEMessage_IEs_t, ie);
    ie->id = NGAP_ProtocolIE_ID_id_TestAdd;
    ie->criticality = NGAP_Criticality_ignore;
    ie->value.present = NGAP_InitialUEMessage_IEs__value_PR_Test_Add;

    size_t new_file_size;

    // Load Key 
    uint8_t* new_file_list = read_uint8_from_bin_file("path to load the encoded_M3.bin", &new_file_size);

    uint8_t special_symbol = 0xFF;
    size_t new_size = 950;

    uint8_t* new_file_list_500 = (uint8_t*) realloc(new_file_list, new_size * sizeof(uint8_t));

    for (size_t i = new_file_size; i < new_size; i++) {
        new_file_list_500[i] = special_symbol;
    }

    NGAP_Test_Add_t new;
    new.buf = new_file_list_500;
    new.size = new_size;

    ie->value.choice.Test_Add = new;
...
//------------------------------------------------------------------------------
{
  ...
  printf("\n");
  printf("\n");
  LOG_W(NGAP,"Do ngap_gNB_nas_uplink End time %ld \n", start_time_ns);
  printf("\n");
  printf("\n");
  printf("\n");
