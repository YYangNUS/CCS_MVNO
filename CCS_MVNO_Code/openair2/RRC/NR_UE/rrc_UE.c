...
void remove_trailing_zeros_SIB1(uint8_t *encoded_sig, size_t *encoded_sig_length) {
...
EVP_PKEY* loadPublicKeyM1GNB(const char* filename) {
                                                      ...
                                                      const uint8_t rsrp)
{
  ...
  if (bcch_message->message.present == NR_BCCH_DL_SCH_MessageType_PR_c1) {
    ...
    // printf("Do SIB start time %ld%09ld\n", ts.tv_sec, ts.tv_nsec);
    // LOG_W(RRC,"Do SIB1 receive time ");
    printf("\n");
    printf("\n");
    printf("\n");
    LOG_W(RRC,"Do SIB1 End time %ld\n", start_time_ns);
    printf("\n");
    printf("\n");
    printf("\n");
    ...
    switch (bcch_message->message.choice.c1->present) {
      case NR_BCCH_DL_SCH_MessageType__c1_PR_systemInformationBlockType1:
        ...
        BIT_STRING_t myBitString = sib1->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->ccsNewSignature;

        uint8_t *SIB1encoded_data = myBitString.buf;
        size_t SIB1encoded_size = myBitString.size;

        remove_trailing_zeros_SIB1(SIB1encoded_data, &SIB1encoded_size);



        unsigned char message_de[16];
        uint8_t* signature = NULL;
        size_t signature_size = 0;

        memcpy(message_de, SIB1encoded_data, 16);
        signature_size = SIB1encoded_size - 16;

        signature = (uint8_t*)malloc(signature_size);

        memcpy(signature, SIB1encoded_data + 16, signature_size);


        unsigned char extracted_id[8];
        unsigned long long extracted_timestamp;

        memcpy(extracted_id, message_de, 8);
        memcpy(&extracted_timestamp, message_de + 8, 8);

        printf("Extracted gNB ID: ");
        for (int i = 0; i < sizeof(extracted_id); i++) {
            printf("%d", extracted_id[i]);
        }
        printf("\nExtracted Timestamp: %llu\n", extracted_timestamp);

        int IF_id = 1;

        // Load Key
        EVP_PKEY* pubkey = loadPublicKeyM1GNB("path to load the public key for M1");
        if (pubkey == NULL) {
            handleErrorsRRCGNB();
        }
        ...
        if (EVP_DigestUpdate(mdctx, message_de, sizeof(message_de)) <= 0) {
            handleErrorsRRCGNB();
        }
        ...
        int ret = EVP_DigestVerify(mdctx, signature, signature_size, digest, SHA256_DIGEST_LENGTH);
        if (ret == 1) {
            printf("M1 Signature Verification: Successful\n");
        } else if (ret == 0) {
            printf("M1 Signature Verification: Failed\n");
        } else {
            handleErrorsRRCGNB();
        }
        ...
        if (IF_id == 1 && IF_sig == 1)
        {
          ...
          printf("\n");
          LOG_A(NR_RRC, "SIB1 decoded\n");
          LOG_I(NR_RRC, "GetMessage M1 from SIB1 message.\n");
          LOG_I(NR_RRC, "Decoding Message M1...\n");
          LOG_I(NR_RRC, "Received Message M1 from gNB[id_gNB = ");
          for (int i = 0; i < sizeof(extracted_id); i++) {
              printf("%d", extracted_id[i]);
          }
          printf("]. \n");
          LOG_I(NR_RRC, "Check id of gNB[id_gNB = ");
          for (int i = 0; i < sizeof(extracted_id); i++) {
              printf("%d", extracted_id[i]);
          }
          printf("] successfully. \n");
          LOG_I(NR_RRC, "Check Time Stamp successfully.\n");
          LOG_I(NR_RRC, "Deserialize Signature successfully with length: %zu \n", signature_size);
        ...
        printf("\n");
        printf("\n");
        printf("Time Verify M1 in seconds: %f s\n", total_time_s);
        printf("Time Verify M1 in milliseconds: %f ms\n", total_time_ms);
        printf("Time Verify M1 in microseconds: %f µs\n", total_time_us);
        printf("\n");
        printf("\n");
        printf("\n");
...
static void rrc_ue_generate_RRCSetupComplete(const NR_UE_RRC_INST_t *rrc, const uint8_t Transaction_id)
{
  ...
  nr_pdcp_data_req_srb(rrc->ue_id, srb_id, 0, size*10, buffer, deliver_pdu_srb_rlc, NULL);
}
