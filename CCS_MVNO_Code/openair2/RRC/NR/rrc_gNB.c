...
void remove_trailing_zeros(uint8_t *encoded_sig, size_t *encoded_sig_length) {
    if (encoded_sig == NULL || encoded_sig_length == NULL || *encoded_sig_length == 0) {
        return;
    }
    
    size_t new_length = *encoded_sig_length;

    while (new_length > 0 && encoded_sig[new_length - 1] == 0) {
        new_length--;
    }

    *encoded_sig_length = new_length;
}

void handleErrorsM22() {
    ERR_print_errors_fp(stderr);
    abort();
}


EVP_PKEY* loadPublicKeyM22(const char* filename) {
...
EVP_PKEY* loadPrivateKeyM22(const char* filename) {
    FILE* fp = fopen(filename, "rb");
    if (fp == NULL) {
        perror("Unable to open the public key");
        return NULL;
    }
...
int decode_parameters_m22(uint8_t *encoded_data, size_t encoded_size, uint64_t *timestamp, uint8_t **proof_buffer, size_t *read_size, unsigned char **serialized_key, int *serialized_len) {
    if (encoded_data == NULL || encoded_size < 20) {
        return 0;
    }
    ...
    memcpy(read_size, p, 8);
    p += 8;

    if (*read_size > (encoded_size - (p - encoded_data) - 4)) { 
        return 0;
    }

    *proof_buffer = (uint8_t *)malloc(*read_size);
    if (*proof_buffer == NULL) {
        return 0;
    }
    memcpy(*proof_buffer, p, *read_size);
    p += *read_size;

    memcpy(serialized_len, p, 4);
    p += 4;

    if (*serialized_len > (encoded_size - (p - encoded_data))) {
        free(*proof_buffer);
        return 0;
    }
    ...
    *serialized_key = (unsigned char *)malloc(*serialized_len);
    ...
    memcpy(*serialized_key, p, *serialized_len);
...
uint64_t get_current_timestamp_ms_m3() {
    struct timeval tv;
...
void encodeM3(uint8_t *encoded, uint8_t signature_size, uint8_t *signature, uint8_t *message, size_t message_len) {
    encoded[0] = signature_size;
    
    memcpy(&encoded[1], signature, signature_size);
    
    memcpy(&encoded[1 + signature_size], message, message_len);
}



int write_uint8_to_bin_file(const char* filename, uint8_t* data, size_t size) {
    ...
    size_t written_size = fwrite(data, sizeof(uint8_t), size, file);
...
int encode_parameters_m22_update(uint64_t timestamp, uint8_t *proof_buffer, size_t read_size, unsigned char *serialized_key, int serialized_len, uint8_t **out, size_t *out_size) {
    size_t total_size = 8 + 4 + read_size + 4 + serialized_len;
    *out = (uint8_t *)malloc(total_size);
    ...
    uint32_t read_size_uint32 = (uint32_t)read_size;
    memcpy(p, &read_size_uint32, 4);
    p += 4;

    memcpy(p, proof_buffer, read_size);
    p += read_size;

    uint32_t serialized_len_uint32 = (uint32_t)serialized_len;
    memcpy(p, &serialized_len_uint32, 4);
    p += 4;
...
//-----------------------------------------------------------------------------
{
  ...
  LOG_W(NR_RRC, "-------------------------gNB Processing RRCSetupComplete  ---------------------------\n");


  uint8_t *encoded_data = rrcSetupComplete->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->ccsNewMessageM2.buf;
  size_t encoded_size = rrcSetupComplete->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->ccsNewMessageM2.size;

  remove_trailing_zeros(encoded_data, &encoded_size);


  uint8_t *sig_contain = rrcSetupComplete->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->ccsNewSignatureM2.buf;
  size_t sig_len_contain = rrcSetupComplete->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->ccsNewSignatureM2.size;



  remove_trailing_zeros(sig_contain, &sig_len_contain);

  int IF_RRC = 1;

  // Load Key
  EVP_PKEY* pubkey = loadPublicKeyM22("path to ec public key for m2");
  if (pubkey == NULL) {
      handleErrorsM22();
  }
  ...
  int ret = EVP_DigestVerify(mdctx, sig_contain, sig_len_contain, digest, SHA256_DIGEST_LENGTH);
  if (ret == 1) {
      printf("Signature M2 Verification: Successful\n");
  } else if (ret == 0) {
      printf("Signature M2 Verification: Failed\n");
  } else {
      handleErrorsM22();
  }
  ...
  uint8_t *decoded_proof_buffer;
  size_t decoded_read_size;
  unsigned char *decoded_serialized_key;
  int decoded_serialized_len;

  if (decode_parameters_m22(encoded_data, encoded_size, &decoded_timestamp, &decoded_proof_buffer, &decoded_read_size, &decoded_serialized_key, &decoded_serialized_len)) {
      printf("Decode Succefful\n");

      printf("%" PRIu64 " decoded_timestamp \n", decoded_timestamp);
  ...
  uint64_t timestamp_m3 = get_current_timestamp_ms_m3();

  uint8_t *encoded_data_m3 = NULL;
  size_t encoded_size_m3 = 0;


  if (encode_parameters_m22_update(timestamp_m3, decoded_proof_buffer, decoded_read_size, decoded_serialized_key, decoded_serialized_len, &encoded_data_m3, &encoded_size_m3)) {

  } else {
      printf("M3 Encode Failed\n");
  }
  ...
  // Load Key
  EVP_PKEY *pkey = loadPrivateKeyM22("path to private key for m3");
  if (pkey == NULL) {
      handleErrorsM22();
  }

  unsigned char digest_new[SHA256_DIGEST_LENGTH];
  EVP_MD_CTX *mdctx_new = EVP_MD_CTX_new(); 
  if (mdctx_new == NULL) {
      handleErrorsM22();
  }

  // SHA-256
  if (EVP_DigestInit_ex(mdctx_new, EVP_sha256(), NULL) <= 0) {
      handleErrorsM22();
  }

  if (EVP_DigestUpdate(mdctx_new, encoded_data_m3, sizeof(encoded_data_m3)) <= 0) {
      handleErrorsM22();
  }

  if (EVP_DigestFinal_ex(mdctx_new, digest_new, NULL) <= 0) {
      handleErrorsM22();
  }
  ...
  // Init
  if (EVP_DigestSignInit(mdctx_new, NULL, EVP_sha256(), NULL, pkey) <= 0) {
      handleErrorsM22();
  }

  if (EVP_DigestSign(mdctx_new, NULL, &sig_len, digest_new, SHA256_DIGEST_LENGTH) <= 0) {
      handleErrorsM22();
  }
  ...
  if (EVP_DigestSign(mdctx_new, sig, &sig_len, digest_new, SHA256_DIGEST_LENGTH) <= 0) {
      handleErrorsM22();
  }

  printf("\n M3 Signature Length: %ld\n", sig_len);


  uint8_t encoded[1 + sig_len + encoded_size_m3];

  encodeM3(encoded, sig_len, sig, encoded_data_m3, encoded_size_m3);

  // Load Key 
  write_uint8_to_bin_file("path to store encoded_M3.bin", encoded, sizeof(encoded));

  printf("\n");
  printf("Size of Encoded %zu", sizeof(encoded));
  ...
  if (IF_RRC == 1 && IF_Verify == 1 && IF_Gen_M3 == 1)
  {
    printf("\n");
    printf("\n");
    printf("\n");
    LOG_A(NR_RRC, "RRC Setup Complete Message Decoded. \n");
    LOG_I(NR_RRC, "Get Message M2 from RRC Setup Complete Message. \n");
    LOG_I(NR_RRC, "Check Time Stamp Successfully. \n");
    LOG_I(NR_RRC, "Verify Signature. \n");
    LOG_A(NR_RRC, "Signature Verification Succeffully. \n");
    LOG_I(NR_RRC, "Generating Signature For Message M3 in Initial UE MEssage. \n");
    LOG_A(NR_RRC, "Finish Generating Message M3 in Initial UE Message. \n");
    printf("\n");
    printf("\n");
    printf("\n");
  }
  ...
  printf("\n");
  printf("\n");
  printf("Time in Gen M3 seconds: %f s\n", total_time_s);
  printf("Time in Gen M3 milliseconds: %f ms\n", total_time_ms);
  printf("Time in Gen M3 microseconds: %f µs\n", total_time_us);
  printf("\n");
  printf("\n");
  printf("\n");
