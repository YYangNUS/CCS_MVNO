...
EVP_PKEY* loadPrivateKeyM1(const char* filename) {
...
void fillBitString_SIB1(BIT_STRING_t *bitString, const uint8_t *data, size_t dataSize, int unusedBits) {
                                      ...
                                      const nr_mac_timers_t *timer_config)
{
  ...
  printf("\n");
  printf("\n");
  LOG_W(NR_RRC,"Do ngap_gNB_nas_uplink End time %ld \n", start_time_ns);
  printf("\n");
  printf("\n");
  printf("\n");
  ...
  *ZoneEight = 0;
  ...
  LOG_I(RRC,"------------------------------------------------------function do SIB1------------------------------------------------------ \n");


  sib1->nonCriticalExtension = CALLOC(1,sizeof(struct NR_SIB1_v1610_IEs));
  AssertFatal(sib1->nonCriticalExtension != NULL, "out of memory\n");

  sib1->nonCriticalExtension->nonCriticalExtension = CALLOC(1,sizeof(struct NR_SIB1_v1630_IEs));
  AssertFatal(sib1->nonCriticalExtension->nonCriticalExtension != NULL, "out of memory\n");

  sib1->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension = CALLOC(1,sizeof(struct NR_SIB1_v1700_IEs));
  AssertFatal(sib1->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension != NULL, "out of memory\n");

  sib1->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension = CALLOC(1,2048);
  AssertFatal(sib1->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension != NULL, "out of memory\n");
  ...
  printf("\nMessage Timestamp: %" PRIu64 " \n", timestamp);

  int IF_sig = 0;

  // Load PrivateKey for M1 Key From keys
  EVP_PKEY *pkey = loadPrivateKeyM1("path to private key");
  if (pkey == NULL) {
      handleErrorsRRCUE();
  }
  ...
  if (EVP_DigestUpdate(mdctx, message, sizeof(message)) <= 0) {
      handleErrorsRRCUE();
  }
  ...
  serialized_size = 16 + sig_len;

  serialized_data = (unsigned char*)malloc(serialized_size);

  memcpy(serialized_data, message, 16);
  memcpy(serialized_data + 16, sig, sig_len);



  BIT_STRING_t myBitString;

  fillBitString_SIB1(&myBitString, serialized_data, serialized_size, 0);

  sib1->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->nonCriticalExtension->ccsNewSignature = myBitString;
  ...
  printf("Time in microseconds: %f µs\n", total_time_us);
  printf("\n");
  printf("\n");
  printf("\n");

  // To better show the output, print them together
  if (IF_certificate == 1 && IF_sig == 1)
  {
    printf("\n");
    printf("\n");
    printf("\n");
    LOG_I(RRC, "gNB[id_gNB = ");
    for (int i = 0; i < sizeof(id); i++) {
        printf("%d", id[i]);
    }
    printf("], Generating Message M1 in SIB1 message.\n");
    LOG_I(RRC, "Generating Certificate.\n");
    LOG_I(RRC, "Generating Signature using SanSig.Sanit.\n");
    LOG_I(RRC, "Generating Signature succeffully with length: %zu \n", sig_len);
    LOG_A(RRC, "Finish Generating Message M1 in SIB1 message. \n");
    printf("\n");
    printf("\n");
    printf("\n");
  }
